...
同期 API メソッドでは、API エラーは HTTP ステータスに応じた `Studio\Auth\ApiException` のサブクラスとして送出されます（未マップのステータスや接続エラーは基底の `ApiException` のまま）:
...
`ApiException::getProblem()` は RFC 9457 Problem Details（`getType()` / `getTitle()` / `getStatus()` / `getDetail()`）を返しますが、内部のデシリアライズは best-effort です。ボディが空のとき（接続エラー等）、またはデシリアライズに失敗したとき（例: `{"status":999}` のように制約に反する値でボディが解釈できないとき）は `null` を返します。一方、非 JSON や OAuth の `{"error": ...}` 形式のような Problem Details でないボディでも、デシリアライズが通れば**フィールドが埋まらない（または同名フィールドだけ埋まった）非 null の `ProblemDetails`** を返すことがあります。いずれの場合も `getProblem()` の戻り値だけでは構造化エラーの有無を判定できないため、**エラー種別の判定には例外のサブクラス（`NotFoundException` 等）または `getCode()`（HTTP ステータスコード）を使ってください**。
...
このパッケージは [Studio Auth Service の OpenAPI 仕様](https://github.com/studio-design/studio-auth) から自動生成されています（SDK バージョン: `0.3.2-rc.21`）。
